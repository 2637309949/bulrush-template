...
type Model struct {
	...
	Creator      *User         `bson:"Creator" br:"ref(user,_creator,_id),comment:'创建人',up(password)"`
	ModifierID   bson.ObjectId `bson:"_modifier,omitempty" br:"comment:'修改人ID'"`
	Modifier     *User         `bson:"Modifier" br:"ref(user,_modifier,_id),comment:'修改人',up(password)"`
	DeleterID    bson.ObjectId `bson:"_deleter,omitempty" br:"comment:'删除人ID'"`
	Deleter      *User         `bson:"Deleter"  br:"ref(user,_deleter,_id),comment:'删除人',up(password)"`
}
